...
MI_S32 MI_AO_SendFrame(MI_AUDIO_DEV AoDevId, MI_AO_CHN AoChn,
  ...
  static MI_S32 (*_MI_AO_SendFrame)(MI_AUDIO_DEV AoDevId, MI_AO_CHN AoChn,
                                    MI_AUDIO_Frame_t * pstData,
                                    MI_S32 s32MilliSec);
  static MI_S32 (*_MI_AO_QueryChnStat)(MI_AUDIO_DEV AoDevId, MI_AO_CHN AoChn,
  ...
  if (s32MilliSec) {
    ...
    if (status.u32ChnBusyNum > BUSY_THRESHOLD) {
      useconds_t usleepclock =
          (uint64_t)(status.u32ChnBusyNum - BUSY_THRESHOLD) * 1000000 /
          (FREQ << 2);
      if (usleepclock)
        usleep(usleepclock);
    }
